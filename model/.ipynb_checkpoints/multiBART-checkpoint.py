...
from generation_utils import BartGenerationMixin
...
class multiDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.encoder_attn_node =  self.encoder_attn
    ...
    def reload_module_dict(self):
        self.encoder_attn_node = deepcopy(self.encoder_attn)
        self.encoder_attn_node.cache_key = "encoder_decoder_p" 
        self.encoder_dict = {'node':self.encoder_attn_node,'source':self.encoder_attn}
    ...
    def forward(
        ...
        for key in MULTI_INPUTS:
            ...
            x_attn_output[key] = F.dropout(x_attn_output[key], p=self.dropout, training=self.training)
...
class multiBartDecoder(nn.Module):
    ...
    def forward(
        ...
        **unused,
        ...
        x = x.transpose(0, 1)
        for key in MULTI_INPUTS:
            encoder_hidden_states[k] = encoder_hidden_states[k].transpose(0, 1)
...
class MultiBartForConditionalGeneration(BartForConditionalGeneration,BartGenerationMixin):
    base_model_prefix = "model"
    authorized_missing_keys = [r"final_logits_bias", r"encoder\.version", r"decoder\.version"]

    def __init__(self, config: BartConfig):
        super().__init__(config)
        base_model = multiBartModel(config)
        self.model = base_model
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
    def reload_module_dict(self):
        self.model.reload_module_dict()
