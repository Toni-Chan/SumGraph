...
MULTI_INPUTS=['source','node']

class BartGenerationMixin:
    """
    ...
    def _prepare_input_ids_for_generation_multi(self, bos_token_id: int) -> torch.LongTensor:
        if bos_token_id is None:
            raise ValueError("`bos_token_id` has to be defined when no `input_ids` are provided.")
        input_ids={}
        for key in MULTI_INPUTS:
            input_ids[key] = torch.ones((1, 1), dtype=torch.long, device=self.device) * bos_token_id
        return input_ids

    def _prepare_attention_mask_for_generation_multi(
        ...
        for key in MULTI_INPUTS:
            if is_pad_token_in_inputs_ids and is_pad_token_not_equal_to_eos_token_id:
                attn_mask[key] = input_ids.ne(pad_token_id).long()
            else:
                attn_mask[key] = input_ids.new_ones(input_ids['source'].shape)
        return attn_mask
    
        
    
    def _prepare_encoder_decoder_kwargs_for_generation_multi(
        ...
        for key in MULTI_INPUTS:
            encoder_outputs[key] = self.model.encoder(
    ...
    def _prepare_decoder_input_ids_for_generation_multi(
        ...
        decoder_input_ids = (
            torch.ones((input_ids['source'].shape[0], 1), dtype=input_ids['source'].dtype, device=input_ids['source'].device)
    ...
    @staticmethod
    def _expand_inputs_for_generation_multi(
        ...
        **model_kwargs
        ...
        for key in MULTI_INPUTS:
            if attention_mask is not None:
                model_kwargs["attention_mask"][key] = attention_mask[key].index_select(0, expanded_return_idx)

        if is_encoder_decoder:
            assert encoder_outputs is not None
            for key in MULTI_INPUTS:
                encoder_outputs[key]["last_hidden_state"] = encoder_outputs[key].last_hidden_state.index_select(
    ...
    def generate_multi(
        ...
        **model_kwargs
        ...
        if input_ids is None:
            ...
            input_ids = self._prepare_input_ids_for_generation_multi(bos_token_id)
        if model_kwargs.get("attention_mask", None) is None:
            # init `attention_mask` depending on `pad_token_id`
            model_kwargs["attention_mask"] = self._prepare_attention_mask_for_generation_multi(
        ...
        if self.config.is_encoder_decoder:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation_multi(input_ids, model_kwargs)

            # set input_ids as decoder_input_ids
            input_ids = self._prepare_decoder_input_ids_for_generation_multi(
        ...
        if is_greedy_gen_mode:
            ...
            # greedy search
            return self.greedy_search_multi(
                input_ids,
            ...
            input_ids, model_kwargs = self._expand_inputs_for_generation_multi(
                input_ids, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder, **model_kwargs
            )
            return self.beam_search_multi(
                input_ids,
                beam_scorer,
                logits_processor=logits_processor,
                max_length=max_length,
    ...
    def beam_search_multi(
        ...
        **model_kwargs
        ...
        Examples::
            ...
            >>> outputs = model.beam_search_multi(input_ids, beam_scorer, logits_processor=logits_processor, **model_kwargs)
